...
public class QnaBoardApplication {
	...
	public Docket newsApi() {
		return new Docket(DocumentationType.SWAGGER_2).groupName("youmekko").apiInfo(apiInfo()).select()
				.paths(PathSelectors.ant("/api/**")).build();
	}

	private ApiInfo apiInfo() {
		return new ApiInfoBuilder().title("Youmekko API")
				.description("Youmekko API")
